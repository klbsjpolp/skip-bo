...
/**
 * Produce the redacted, viewer-relative game view for a single seat. Run by the
 * host-authoritative client for every seat (including itself). Opponent hands
 * and hidden stock cards are replaced with HIDDEN_CARD so the wire never carries
 * an opponent's secret information.
 */
