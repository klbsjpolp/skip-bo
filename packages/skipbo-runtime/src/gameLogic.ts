...
const isSupportedOnlineAction = (action: GameAction, allowDebug: boolean): boolean => {
  if (
    action.type === 'INIT' ||
    action.type === 'DRAW' ||
...
export interface OnlineInitialGameStateOptions {
  playerCount?: number;
...
/**
 * Validate a move against the authoritative game state. Returns an error message
 * or `null`. `allowDebug` gates the DEBUG_* actions (host enables them only in
 * non-production builds).
 */
export const validateOnlineAction = (gameState: GameState, action: GameAction, allowDebug: boolean): string | null => {
  if (!isSupportedOnlineAction(action, allowDebug)) {
...
/**
 * Apply a validated move to the authoritative game state, including the
 * start-of-turn draw that happens when the turn advances.
 */
