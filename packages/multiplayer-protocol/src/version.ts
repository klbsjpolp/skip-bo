...
// Clients that omit `protocolVersion` are treated as v1 — that's the version
// pre-handshake builds were shipping when this field was introduced. As soon
// as MIN_SUPPORTED_PROTOCOL_VERSION moves past 1 those absent-field clients
// will (correctly) be rejected; the schema keeps the field `.optional()` only
// so the gate logic lives here in one place.
