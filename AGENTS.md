...
| Realtime relay protocol or room lifecycle       | Lives in [realtime-infra](https://github.com/klbsjpolp/realtime-infra) (`@klbsjpolp/realtime-core` + the relay server). This repo only consumes the published `@klbsjpolp/realtime-core`. | Change the protocol/server in realtime-infra. Here, bump the `@klbsjpolp/realtime-core` version and run a two-browser smoke test.                                                                                                                                           | [docs/architecture/online-multiplayer.md](docs/architecture/online-multiplayer.md), and the decision log if the architecture changed.                                                                             |
| Host-authoritative Skip-Bo runtime or redaction | `packages/skipbo-runtime/src/{hostRuntime,gameLogic,views,actionSchema}.ts`                                                                                                               | Update `packages/skipbo-runtime/tests`. Run a two-browser smoke test (hidden-hand leak check) when redaction or rules changed.                                                                                                                                              | [docs/protocols/realtime-events.md](docs/protocols/realtime-events.md) and [docs/architecture/online-multiplayer.md](docs/architecture/online-multiplayer.md).                                                    |
