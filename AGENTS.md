...
| Realtime relay protocol or room lifecycle       | `apps/realtime-api/src/services/roomService.ts`, `packages/realtime-core/src/index.ts`                                                         | Update `apps/realtime-api/src/**/__tests__` and `packages/realtime-core/tests`. Run a two-browser smoke test when behavior changed.                                                                                                                                         | [docs/protocols/realtime-events.md](docs/protocols/realtime-events.md), [docs/architecture/online-multiplayer.md](docs/architecture/online-multiplayer.md), and the decision log if the architecture changed.                                               |
...
- If relay DTOs, room codes, or WebSocket message shapes changed, update `packages/realtime-core/tests` and `apps/realtime-api/src/**/__tests__`. If Skip-Bo redaction or move rules changed, update `packages/skipbo-runtime/tests`.
