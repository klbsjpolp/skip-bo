...
import { aiCoachRequestSchema } from '@skipbo/multiplayer-protocol';
import { isClientError } from '../../errors/clientError.js';
import { captureBackendException, withSentry } from '../../monitoring/sentry.js';
import { requestAiCoach } from '../../services/aiInsightsService.js';
import { jsonResponse } from '../../utils/http.js';
import { roomRepository } from '../shared.js';

const aiCoachHandler: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    const body: unknown = event.body ? JSON.parse(event.body) : {};
    const request = aiCoachRequestSchema.parse(body);
    const response = await requestAiCoach({ roomRepository }, request);
...
export const handler = withSentry(aiCoachHandler);
