...
function getTextAndColourForCard(
  card: CardType | null,
  overriddenDisplayValue: string | undefined,
  isRevealed: boolean,
) {
  if (!card || !isRevealed) return { colourClass: '', cardValue: '' };
  const cardValue =
    overriddenDisplayValue === undefined
      ? card.isSkipBo
        ? 'Skip-Bo'
        : (card.value?.toString() ?? '')
      : overriddenDisplayValue;
  const colourClass =
    cardValue === 'Skip-Bo'
      ? 'skipbo-text'
      : Number(cardValue) <= 4
        ? 'card-range-1 normal-card'
        : Number(cardValue) <= 8
          ? 'card-range-2 normal-card'
          : Number(cardValue) <= 12
            ? 'card-range-3 normal-card'
            : '';
...
const CardComponent: React.FC<CardProps> = ({
  ...
  const [morphing, setMorphing] = useState<'no' | 'yes' | 'after'>(isMorphCandidate ? 'yes' : 'no');
  ...
  const { colourClass, cardValue } = getTextAndColourForCard(
    card,
    morphing === 'yes' ? undefined : overriddenDisplayValue,
    isRevealed,
  );
  ...
  if (card && shouldMorph) {
    ...
    return (
      <div className="card-morph-wrapper" style={style} {...interactiveProps} {...extraProps}>
        {/* FROM (Skip-Bo) layer */}
        <div
          className={cn('card', 'card-morph-layer', className, fromData.colourClass)}
        ...
        {/* TO (new value) layer */}
        <div
          className={cn('card', 'card-morph-layer', className, toData.colourClass)}
