...
const urlMatchesTracePropagationTarget = (targets: Array<string | RegExp>, url: string): boolean =>
...
describe('getSentryTracePropagationTargets', () => {
  ...
  it('matches deployed API Gateway URLs', () => {
    ...
    expect(
      urlMatchesTracePropagationTarget(targets, 'https://abc123.execute-api.ca-central-1.amazonaws.com/rooms'),
    ).toBe(true);
  });
  ...
  it('adds a custom API origin when the build config uses a non-AWS domain', () => {
    ...
    expect(urlMatchesTracePropagationTarget(targets, 'https://api.skipbo.example.com/rooms/join')).toBe(true);
  });
});
