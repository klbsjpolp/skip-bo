...
- The Skip-Bo card is the leader frame before the picture starts — the same silver-nitrate print as the numbered cards (ink crosshair and circles, italic ink wordmark) but with no wipe: the count at zero. Playing it on a build pile morphs it into the value's wipe.
