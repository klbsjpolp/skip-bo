...
- The Skip-Bo card is an intertitle title card — black face, double silver frame (`border` + offset `outline` on `.card-inner::before`), italic serif wordmark — carrying the Academy-leader countdown in negative (silver crosshair and circles, no wipe): the count at zero, before the picture starts. Playing it on a build pile morphs it into the value's wipe.
