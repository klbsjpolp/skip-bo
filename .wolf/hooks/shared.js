...
export function extractDescription(filePath) {
  ...
  if (ext === '.php') {
    ...
    if (parent === 'Model' || parent === 'Authenticatable') {
      ...
      const rels = (content.match(/\$this->(hasMany|hasOne|belongsTo|belongsToMany|morphMany|morphTo)\(/g) || [])
        .length;
    ...
    if (className && pubMethods.length > 0) {
      ...
      return cap(
        pubMethods.length > 4 ? `${className}: ${display} + ${pubMethods.length - 4} more` : `${className}: ${display}`,
      );
    }
  }
  // ─── TS/JS/React/Next.js ─────────────────────────────────
  if (ext === '.ts' || ext === '.tsx' || ext === '.js' || ext === '.jsx' || ext === '.mjs' || ext === '.cjs') {
    ...
    if (basename === 'route.ts' || basename === 'route.js') {
      const methods = [
        ...new Set(
          (content.match(/export\s+(?:async\s+)?function\s+(GET|POST|PUT|PATCH|DELETE)/g) || []).map(
            (m) => m.match(/(GET|POST|PUT|PATCH|DELETE)/)?.[1],
          ),
        ),
      ].filter(Boolean);
  ...
  if (ext === '.java') {
    ...
    const annotations = (
      content.match(/@(RestController|Controller|Service|Repository|Component|Entity|Configuration)/g) || []
    ).map((a) => a.slice(1));
  ...
  if (ext === '.sql') {
    const creates = (content.match(/CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?[`"']?(\w+)/gi) || [])
      .map((m) => m.match(/(?:TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?)([`"']?\w+)/i)?.[1]?.replace(/[`"']/g, ''))
      .filter(Boolean);
